...
// Render dropdown options with thumbnails (UPDATED: No SKU display)
function renderOptions() {
    ...
    filteredPatterns.forEach((pattern, index) => {
        ...
        // Create text content (UPDATED: Only pattern name, no SKU)
        const textDiv = document.createElement('div');
        textDiv.className = 'option-text';
        textDiv.textContent = pattern.name;
        
        // Assemble option (UPDATED: No SKU span)
...
function setupDropdownEventListeners() {
    ...
    // Search input handler (UPDATED: Search only by name, not SKU)
...
// Handle search input (UPDATED: Search only by name)
function handleSearch(e) {
    ...
    if (searchTerm === '') {
        ...
        filteredPatterns = allPatterns.filter(pattern => {
            ...
            return nameMatch; // Removed SKU matching
        });
    }
...
// Select a pattern (UPDATED: Display only pattern name)
function selectPattern(patternId) {
    ...
    // Update visual display (UPDATED: Only show pattern name)
    const selectedText = document.getElementById('dropdownSelected').querySelector('.selected-text');
    const hiddenInput = document.getElementById('pattern');
    
    selectedText.textContent = pattern.name; // Removed SKU display
